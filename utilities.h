...
bool setupPMU()
{
  ...
  if (readByte(IP5306_ADDR, 0x21, &data) == true) {
    writeByte(IP5306_ADDR, 0x21, (data & 0x3f) | 0x00);
  }

  // Add volt 28mv
  if (readByte(IP5306_ADDR, 0x22, &data) == true) {
    writeByte(IP5306_ADDR, 0x22, (data & 0xfc) | 0x02);
  }

  // Vin charge CC
  if (readByte(IP5306_ADDR, 0x23, &data) == true) {
    writeByte(IP5306_ADDR, 0x23, (data & 0xdf) | 0x20);
  }
  //end from m5stack
  ...
  Wire.write(0x37); // Set bit1: 1 enable boost keep on
